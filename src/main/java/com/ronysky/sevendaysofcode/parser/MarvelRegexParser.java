...
public class MarvelRegexParser implements JsonParser<Serie>{
	
	public Collection<Serie> getContent(String json) {
		String[] seriesArray = parseJsonSeries(json);
		
		List<Serie> series = new ArrayList<>();
		for (int i = 0; i < seriesArray.length; i++) {
			String titleValue = parseAttribute(seriesArray[i], "title");
			Integer yearValue = Integer.valueOf(parseAttribute(seriesArray[i], "startYear"));
			...
			series.add(new Serie(titleValue, yearValue, thumbnailValue, ratingValue));
		}
